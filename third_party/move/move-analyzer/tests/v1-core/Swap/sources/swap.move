module SwapDeployer::AnimeSwapPoolV1 {
  ...
  fun init_module(admin: &signer) {
    ...
    // init pair info
    move_to(resource_account, PairInfo {pair_list: vector::empty(),});
  }
  ...
  */
  ...
  public fun get_amounts_out_1_pair<X, Y>(amount_in: u64): u64
    acquires LiquidityPool, AdminData {
  ...
  public fun get_amounts_out_2_pair<X, Y, Z>(amount_in: u64): u64
    acquires LiquidityPool, AdminData {
    let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
    let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
  ...
  public fun get_amounts_out_3_pair<X, Y, Z, W>(amount_in: u64): u64
    acquires LiquidityPool, AdminData {
    let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
    let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
    let amount_mid = AnimeSwapPoolV1Library::get_amount_out(amount_in, reserve_in, reserve_out, swap_fee);
  ...
  public fun get_amounts_in_1_pair<X, Y>(amount_out: u64): u64
    acquires LiquidityPool, AdminData {
    let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
    let (reserve_in, reserve_out) = get_reserves_size<X, Y>();
  ...
  public fun get_amounts_in_2_pair<X, Y, Z>(amount_out: u64): u64
    acquires LiquidityPool, AdminData {
    let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
  ...
  public fun get_amounts_in_3_pair<X, Y, Z, W>(amount_out: u64): u64
    acquires LiquidityPool, AdminData {
    let swap_fee = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).swap_fee;
  ...
  fun assert_paused()
    acquires AdminData {
    assert !(
      borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS).is_pause,
      ERR_PAUSABLE_ERROR
    );
  }
  ...
  public fun calc_optimal_coin_values<X, Y>(
    ...
    if (reserve_x == 0 && reserve_y == 0) {(amount_x_desired, amount_y_desired) } else {
  ...
  fun assert_k_increase(
    ...
    // gas saving
    if (
      AnimeSwapPoolV1Library::is_overflow_mul(balance_x_adjusted, balance_y_adjusted) || AnimeSwapPoolV1Library::is_overflow_mul(
        balance_xy_old_not_scaled, scale
      )) {
      let balance_xy_adjusted = u256::mul(
        u256::from_u128(balance_x_adjusted),
        u256::from_u128(balance_y_adjusted)
      );
      let balance_xy_old = u256::mul(u256::from_u128(balance_xy_old_not_scaled), u256::from_u128(scale));
      assert !(
        u256::compare(&balance_xy_adjusted,&balance_xy_old) == 2,
        ERR_K_ERROR
      );
    } else {
  ...
  fun update_internal<X, Y>(
    ...
    if (time_elapsed > 0 && reserve_x != 0 && reserve_y != 0) {
      ...
      lp.last_price_x_cumulative = AnimeSwapPoolV1Library::overflow_add(
        lp.last_price_x_cumulative, last_price_x_cumulative_delta
      );

      let last_price_y_cumulative_delta = uq64x64::to_u128(uq64x64::fraction(reserve_x, reserve_y)) * time_elapsed;
      lp.last_price_y_cumulative = AnimeSwapPoolV1Library::overflow_add(
        lp.last_price_y_cumulative, last_price_y_cumulative_delta
      );
    };
  ...
  fun mint_fee_interval<X, Y>(lp: &mut LiquidityPool<X, Y>, admin_data: &AdminData): bool {
        ...
        if (root_k > root_k_last) {
          ...
          if (AnimeSwapPoolV1Library::is_overflow_mul(total_supply, delta_k)) {
            ...
            let denominator = u256::from_u128(
              (root_k as u128) * (admin_data.dao_fee as u128) + (root_k_last as u128)
            );
  ...
  fun mint_coin<X, Y>(account: &signer, amount: u64, mint_cap: &MintCapability<LPCoin<X, Y>>) {
  ...
  */
  ...
  public entry fun swap_exact_coins_for_coins_entry<X, Y>(account: &signer, amount_in: u64, amount_out_min: u64,)
    ...
    assert !(
      coin::value(&coins_out) >= amount_out_min,
  ...
  */
  ...
  public entry fun set_dao_fee_to(account: &signer, dao_fee_to: address)
    acquires AdminData {
    let admin_data = borrow_global_mut<AdminData>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      signer::address_of(account) == admin_data.admin_address,
      ERR_FORBIDDEN
    );
    admin_data.dao_fee_to = dao_fee_to;
  }

  public entry fun set_admin_address(account: &signer, admin_address: address)
    acquires AdminData {
    let admin_data = borrow_global_mut<AdminData>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      signer::address_of(account) == admin_data.admin_address,
  ...
  public entry fun set_dao_fee(account: &signer, dao_fee: u8)
    acquires AdminData {
    let admin_data = borrow_global_mut<AdminData>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      signer::address_of(account) == admin_data.admin_address,
  ...
  public entry fun set_swap_fee(account: &signer, swap_fee: u64)
    acquires AdminData {
    let admin_data = borrow_global_mut<AdminData>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      signer::address_of(account) == admin_data.admin_address,
  ...
  public entry fun withdraw_dao_fee<X, Y>(account: &signer)
    acquires AdminData {
    if (!AnimeSwapPoolV1Library::compare<X, Y>()) {
  ...
  */
  ...
  public fun create_pair<X, Y>()
    ...
    event::emit_event(&mut events.pair_created_event, PairCreatedEvent {meta: pair_meta,});
  ...
  */
  ...
  /// Mint new LPCoin
  public fun mint<X, Y>(coin_x: Coin<X>, coin_y: Coin<Y>): Coin<LPCoin<X, Y>>
    acquires LiquidityPool, AdminData, Events {
    assert !(AnimeSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
    assert !(
      exists<LiquidityPool<X, Y>>(RESOURCE_ACCOUNT_ADDRESS),
    ...
    let events = borrow_global_mut<Events<X, Y>>(RESOURCE_ACCOUNT_ADDRESS);
    event::emit_event(
      &mut events.mint_event,
      MintEvent {amount_x, amount_y, liquidity,}
    );
    coins
  }

  /// Burn LPCoin and get back coins
  public fun burn<X, Y>(liquidity: Coin<LPCoin<X, Y>>): (Coin<X>, Coin<Y>)
    acquires LiquidityPool, AdminData, Events {
    assert !(AnimeSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
    assert_lp_unlocked<X, Y>();
  ...
  */
  ...
  public fun get_last_price_cumulative<X, Y>(): (u128, u128, u64)
    acquires LiquidityPool {
    if (AnimeSwapPoolV1Library::compare<X, Y>()) {
      let lp = borrow_global<LiquidityPool<X, Y>>(RESOURCE_ACCOUNT_ADDRESS);
      (
        lp.last_price_x_cumulative,
        lp.last_price_y_cumulative,
        lp.last_block_timestamp
      )
    } else {
      let lp = borrow_global<LiquidityPool<Y, X>>(RESOURCE_ACCOUNT_ADDRESS);
      (
  ...
  public fun get_admin_data(): (u64, u8, bool, bool)
    acquires AdminData {
    let admin_data = borrow_global<AdminData>(RESOURCE_ACCOUNT_ADDRESS);
    (admin_data.swap_fee, admin_data.dao_fee, admin_data.dao_fee_on, admin_data.is_pause)
  }

  public fun get_pair_list(): vector<PairMeta>
    acquires PairInfo {
  ...
  */
  ...
  public fun flash_swap<X, Y>(loan_coin_x: u64, loan_coin_y: u64): (
    Coin<X>,
    Coin<Y>,
    FlashSwap<X, Y>
  )
    ...
    let lp = borrow_global_mut<LiquidityPool<X, Y>>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      coin::value(&lp.coin_x_reserve) >= loan_coin_x && coin::value(&lp.coin_y_reserve) >= loan_coin_y, ERR_INSUFFICIENT_AMOUNT
    );
    lp.locked = true;
    ...
    (
      loaned_coin_x,
      loaned_coin_y,
      FlashSwap<X, Y>{loan_coin_x, loan_coin_y}
    )
  }
  ...
  #[test_only]
  fun init_module_test(resource_account: &signer)
    acquires AdminData, PairInfo {
    move_to(
      resource_account,
      AdminData {
  ...
  #[test_only]
  fun test_init(creator: &signer, resource_account: &signer, someone_else: &signer)
    acquires AdminData, PairInfo {
    genesis::setup();
...

    {
      ...
      let (apt_b, apt_f, apt_m) = coin::initialize<BTC>(
        creator,
        utf8(b"Bitcoin"),
        utf8(b"BTC"),
        6,
...

    {
      ...
      let (apt_b, apt_f, apt_m) = coin::initialize<USDT>(
        creator,
        utf8(b"Tether"),
        utf8(b"USDT"),
        6,
...

    {
      ...
      let (apt_b, apt_f, apt_m) = coin::initialize<Aptos>(
        creator,
        utf8(b"Aptos"),
        utf8(b"APT"),
        6,
...

    {
      ...
      let (apt_b, apt_f, apt_m) = coin::initialize<AptosB>(
        creator,
        utf8(b"AptosB"),
        utf8(b"APTB"),
        6,
  ...
  #[test_only]
  fun test_init_another_one(resource_account: &signer, another_one: &signer)
    acquires Caps {
  ...
  public entry fun test_add_remove_liquidity_basic_1_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    add_liquidity_entry<BTC, USDT>(someone_else, 10000, 10000, 1, 1);
    {
      ...
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 9000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    add_liquidity_entry<BTC, USDT>(someone_else, 1000, 100, 1, 1);
    {
      ...
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 9100,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10100,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10100,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    remove_liquidity_entry<BTC, USDT>(someone_else, 9000, 9000, 9000);
    {
      ...
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 100,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 1100,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 1100,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_add_remove_liquidity_basic_1_100(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    remove_liquidity_entry<BTC, USDT>(someone_else, 9000, 900, 90000);
    {
      ...
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 110,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 11000,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_add_remove_liquidity_basic_1_2(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    add_liquidity_entry<BTC, USDT>(someone_else, 1000, 2000, 1, 1);
    {
      ...
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 414,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    add_liquidity_entry<BTC, USDT>(someone_else, 2000, 2000, 1, 1);
    {
      ...
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 1828,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    remove_liquidity_entry<BTC, USDT>(someone_else, 1828, 1, 1);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 2000 - 1292,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 4000 - 2585,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 0,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - (2000 - 1292),
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - (4000 - 2585),
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_basic_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_exact_coins_for_coins_entry<BTC, USDT>(someone_else, 1000, 1);
    {
      ...
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 - 1000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 + 906,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    swap_exact_coins_for_coins_entry<USDT, BTC>(someone_else, 1000, 1);
    {
      ...
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 - 1000 + 1086,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 + 906 - 1000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    remove_liquidity_entry<BTC, USDT>(someone_else, 1000, 1, 1);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 9914 - 991,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 10094 - 1009,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 8000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 - 1000 + 1086 + 991,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 + 906 - 1000 + 1009,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_basic_2(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_coins_for_exact_coins_entry<BTC, USDT>(someone_else, 1000, 100000);
    {
      ...
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 - 1115,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 + 1000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    swap_coins_for_exact_coins_entry<USDT, BTC>(someone_else, 1000, 100000);
    {
      ...
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 - 1115 + 1000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000 + 1000 - 893,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_multiple_pair_1_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_exact_coins_for_coins_2_pair_entry<BTC, USDT, Aptos>(someone_else, 10000, 1);
    {
      ...
      assert !(
        coin::value(&lp.coin_x_reserve) == 10010000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 9990040,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 10009960,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 9990080,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 - 10000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 20000000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<Aptos>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 + 9920,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_multiple_pair_1_2(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_exact_coins_for_coins_3_pair_entry<BTC, USDT, Aptos, AptosB>(someone_else, 10000, 1);
    {
      ...
      assert !(
        coin::value(&lp_3.coin_x_reserve) == 10009920,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_y_reserve) == 9990120,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 - 10000,
      ...
      assert !(
        coin::balance<Aptos>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 20000000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<AptosB>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 + 9880,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_multiple_pair_2_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_coins_for_exact_coins_2_pair_entry<BTC, USDT, Aptos>(someone_else, 10000, 1000000);
    {
      ...
      let lp_2 = borrow_global<LiquidityPool<USDT, Aptos>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 10010082,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 9989959,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 10010041,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 9990000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 - 10082,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 20000000,
      ...
      assert !(
        coin::balance<Aptos>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 + 10000,
        USER_LP_BALANCE_ERROR
      );
    }
  }
  ...
  public entry fun test_swap_multiple_pair_2_1_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_coins_for_exact_coins_2_pair_entry<BTC, USDT, Aptos>(someone_else, 10000, 100000000000000);
    {
      ...
      let lp_2 = borrow_global<LiquidityPool<USDT, Aptos>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 101026651,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(coin::value(&lp.coin_y_reserve) == 989868, CONTRACTOR_BALANCE_ERROR);
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 1010132,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 990000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000000 - 1026651,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 2000000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<Aptos>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 1000000 + 10000,
        USER_LP_BALANCE_ERROR
      );
    }
  }
  ...
  public entry fun test_swap_multiple_pair_2_2(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_coins_for_exact_coins_3_pair_entry<BTC, USDT, Aptos, AptosB>(someone_else, 10000, 1000000);
    {
      ...
      let lp_3 = borrow_global<LiquidityPool<Aptos, AptosB>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 10010123,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 9989918,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 10010082,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 9989959,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_x_reserve) == 10010041,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_y_reserve) == 9990000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 - 10123,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 20000000,
      ...
      assert !(
        coin::balance<AptosB>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 + 10000,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_swap_multiple_pair_2_2_1(creator: &signer, resource_account: &signer, someone_else: &signer)
    ...
    swap_coins_for_exact_coins_3_pair_entry<BTC, USDT, Aptos, AptosB>(someone_else, 1000, 100000000000000);
    {
      ...
      let lp_3 = borrow_global<LiquidityPool<Aptos, AptosB>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 10104130,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(coin::value(&lp.coin_y_reserve) == 989725, CONTRACTOR_BALANCE_ERROR);
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 1010275,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 98986,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_x_reserve) == 101014,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_y_reserve) == 99000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 10000000 - 104130,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 2000000,
      ...
      assert !(
        coin::balance<Aptos>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 200000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<AptosB>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000 + 1000,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_add_multiple_liquidity(creator: &signer, resource_account: &signer, someone_else: &signer)
...

    {
      ...
      assert !(
        coin::value(&lp.coin_x_reserve) == 1000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(coin::value(&lp.coin_y_reserve) == 10000, CONTRACTOR_BALANCE_ERROR);
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 10000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 10000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_x_reserve) == 10000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_y_reserve) == 1000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<USDT, Aptos>>(signer::address_of(someone_else)) == 99000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, Aptos>>(signer::address_of(someone_else)) == 9000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 99000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    assert !(
      coin::value(&lp_1.coin_x_reserve) == 1000000,
      CONTRACTOR_BALANCE_ERROR
    );
    assert !(
      coin::value(&lp_1.coin_y_reserve) == 10000,
      CONTRACTOR_BALANCE_ERROR
    );
    let lp_2 = borrow_global<LiquidityPool<BTC, Aptos>>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
...

    {
      ...
      let lp_3 = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 2000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(coin::value(&lp.coin_y_reserve) == 20000, CONTRACTOR_BALANCE_ERROR);
      assert !(
        coin::value(&lp_2.coin_x_reserve) == 20000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_2.coin_y_reserve) == 20000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_x_reserve) == 20000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp_3.coin_y_reserve) == 2000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<USDT, Aptos>>(signer::address_of(someone_else)) == 199000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, Aptos>>(signer::address_of(someone_else)) == 19000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 199000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    let lp_1 = borrow_global<LiquidityPool<USDT, Aptos>>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
      coin::value(&lp_1.coin_x_reserve) == 2000000,
      CONTRACTOR_BALANCE_ERROR
    );
    assert !(
      coin::value(&lp_1.coin_y_reserve) == 20000,
      CONTRACTOR_BALANCE_ERROR
    );
    let lp_2 = borrow_global<LiquidityPool<BTC, Aptos>>(RESOURCE_ACCOUNT_ADDRESS);
    assert !(
  ...
  public entry fun test_dao_fee(creator: &signer, resource_account: &signer, someone_else: &signer, dao_fee_to: &signer)
    ...
    add_liquidity_entry<BTC, USDT>(someone_else, 100000000, 100000000, 1, 1);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 100000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 100000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 100000000 - 1000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000000,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    swap_exact_coins_for_coins_entry<BTC, USDT>(someone_else, 10000000, 1);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 110000000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 90933892,
        CONTRACTOR_BALANCE_ERROR
      ); // 1e8-floor(1e8-1e8**2/(1e8+0.0997e8))
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 100000000 - 1000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000000 - 10000000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 100000000 + 9066108,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    remove_liquidity_entry<BTC, USDT>(someone_else, 1000000, 1, 1);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 108900076,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 90024616,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(someone_else)) == 98999000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<BTC>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 108900076,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<USDT>(signer::address_of(someone_else)) == INIT_FAUCET_COIN - 90024616,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS) == 1000 + 6819,
        USER_LP_BALANCE_ERROR
      );
    };
    ...
    {
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS) == 1000,
        USER_LP_BALANCE_ERROR
      );
      assert !(
        coin::balance<LPCoin<BTC, USDT>>(signer::address_of(dao_fee_to)) == 6819,
        USER_LP_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_flash_swap_a(creator: &signer, resource_account: &signer, someone_else: &signer, another_one: &signer)
    ...
    pay_flash_swap<BTC, USDT>(coin::zero<BTC>(), repay_coin, flash_swap);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 10000 - 1000,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 100000 + 11145,
        CONTRACTOR_BALANCE_ERROR
      );
    };
  }
  ...
  public entry fun test_flash_swap_b(creator: &signer, resource_account: &signer, someone_else: &signer, another_one: &signer)
    ...
    pay_flash_swap<BTC, USDT>(repay_coin, coin::zero<USDT>(), flash_swap);
    {
      let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
      assert !(
        coin::value(&lp.coin_x_reserve) == 10000 + 102,
        CONTRACTOR_BALANCE_ERROR
      );
      assert !(
        coin::value(&lp.coin_y_reserve) == 100000 - 1000,
        CONTRACTOR_BALANCE_ERROR
      );
    };
  }
