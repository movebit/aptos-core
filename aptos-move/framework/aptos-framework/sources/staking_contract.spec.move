spec aptos_framework::staking_contract {
    ...
    spec schema Create_Staking_Contract_With_Coins_Abortsif {
        ...
        aborts_if amount < min_stake_required; // ? aborts_if amount <= min_stake_required;
        ...
        include Create_Stake_Pool_Aborts_If {resource_addr};
        ...
        let update_voting_power_increase = amount != 0 && (stake::spec_contains(validator_set.active_validators, resource_addr)
                                                           || stake::spec_contains(validator_set.pending_active, resource_addr));
        ...
        let stake_pool = global<stake::StakePool>(resource_addr);
        let post post_stake_pool = global<stake::StakePool>(resource_addr);
        let value_pending_active = stake_pool.pending_active.value;
        let value_active = stake_pool.active.value;
        aborts_if amount != 0 && stake::spec_is_current_epoch_validator(resource_addr) && value_pending_active + amount > MAX_U64;
        ensures amount != 0 && stake::spec_is_current_epoch_validator(resource_addr) ==> post_stake_pool.pending_active.value == value_pending_active + amount;
        aborts_if amount != 0 && !stake::spec_is_current_epoch_validator(resource_addr) && value_active + amount > MAX_U64;
        ensures amount != 0 && !stake::spec_is_current_epoch_validator(resource_addr) ==> post_stake_pool.active.value == value_active + amount;
    ...
    spec create_staking_contract_with_coins(
        ...
        aborts_if principal < min_stake_required; // ? aborts_if principal <= min_stake_required;
        ...
        let voting_power_increase_limit = config.voting_power_increase_limit;
        let post post_validator_set = global<stake::ValidatorSet>(@aptos_framework);
        let update_voting_power_increase = principal != 0 && (stake::spec_contains(validator_set.active_validators, resource_addr)
                                                           || stake::spec_contains(validator_set.pending_active, resource_addr));
        aborts_if update_voting_power_increase && validator_set.total_joining_power + principal > MAX_U128;
        ensures update_voting_power_increase ==> post_validator_set.total_joining_power == validator_set.total_joining_power + principal;
        aborts_if update_voting_power_increase && validator_set.total_voting_power > 0
                && validator_set.total_voting_power * voting_power_increase_limit > MAX_U128;
        aborts_if update_voting_power_increase && validator_set.total_voting_power > 0
                && validator_set.total_joining_power + principal > validator_set.total_voting_power * voting_power_increase_limit / 100;
        let stake_pool = global<stake::StakePool>(resource_addr);
        let post post_stake_pool = global<stake::StakePool>(resource_addr);
        let value_pending_active = stake_pool.pending_active.value;
        let value_active = stake_pool.active.value;
        aborts_if principal != 0 && stake::spec_is_current_epoch_validator(resource_addr) && value_pending_active + principal > MAX_U64;
        ensures principal != 0 && stake::spec_is_current_epoch_validator(resource_addr) ==> post_stake_pool.pending_active.value == value_pending_active + principal;
        aborts_if principal != 0 && !stake::spec_is_current_epoch_validator(resource_addr) && value_active + principal > MAX_U64;
        ensures principal != 0 && !stake::spec_is_current_epoch_validator(resource_addr) ==> post_stake_pool.active.value == value_active + principal;
        let maximum_stake = config.maximum_stake;
        let value_pending_inactive = stake_pool.pending_inactive.value;
        let next_epoch_voting_power = value_pending_active + value_active + value_pending_inactive;
        let voting_power = next_epoch_voting_power + principal;
        aborts_if principal != 0 && voting_power > MAX_U64;
        aborts_if principal != 0 && voting_power > maximum_stake;

        /// 295
        let post post_store = global<Store>(staker_address);
        let post post_staking_contracts = post_store.staking_contracts;
        ensures simple_map::spec_contains_key(post_staking_contracts, operator);
        ensures principal != 0 ==> simple_map::spec_contains_key(post_staking_contracts, operator);
    }
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        let value_pending_active = stake_pool.pending_active.value;
        let value_active = stake_pool.active.value;
        aborts_if amount != 0 && stake::spec_is_current_epoch_validator(pool_address) && value_pending_active + amount > MAX_U64;
        ...
        aborts_if amount != 0 && !stake::spec_is_current_epoch_validator(pool_address) && value_active + amount > MAX_U64;
    ...
    spec create_stake_pool(
        ...
        let staker_address = signer::address_of(staker);
        // include Create_Stake_Pool_Aborts_If {staker_address};
        
        let seed_0 = bcs::to_bytes(staker_address);
        let seed_1 = concat(concat(concat(seed_0, bcs::to_bytes(operator)), SALT), contract_creation_seed);
        let resource_addr = account::spec_create_resource_address(staker_address, seed_1);
        include Create_Stake_Pool_Aborts_If {resource_addr};
        let account = global<account::Account>(resource_addr);
        // aborts_if exists<account::Account>(resource_addr) && (len(account.signer_capability_offer.for.vec) != 0 || account.sequence_number != 0);
        ...
        // aborts_if exists<account::Account>(resource_addr) && account.guid_creation_num + 12  >= account::MAX_GUID_CREATION_NUM;
    ...
    spec schema Create_Stake_Pool_Aborts_If {
